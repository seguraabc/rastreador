...
export default function App() {
  ...
  const [useSimulation, setUseSimulation] = useState<boolean>(false); // Default to Real Mode
  ...
  // Real BLE Logic using Capacitor Plugin
  const startBleScan = async () => {
    try {
      console.log("Initializing BLE Service...");
      const hasPermissions = await BLEService.requestPermissions();
      if (!hasPermissions) {
        alert("No se pudieron obtener los permisos de Bluetooth/Ubicación.");
        setStatus(ScannerStatus.ERROR);
        return;
      }

      await BLEService.startScanning((beaconData) => {
        handleBeaconFound(beaconData);
      });

    } catch (error) {
      console.error("Error starting BLE scan:", error);
      setStatus(ScannerStatus.ERROR);
      alert("Error al iniciar el escaneo BLE: " + (error as any).message);
    }
  };
